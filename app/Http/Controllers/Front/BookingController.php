...
class BookingController extends Controller
{
    ...
    public function cart_submit(Request $request)
    {
        ...
        // Check if the customer is trying to add the same room with overlapping dates
        ...
        foreach ($cart_rooms as $index => $cart_room_id) {
            if ($cart_room_id == $request->room_id) {
                $existing_checkin = strtotime($cart_checkin_dates[$index]);
                $existing_checkout = strtotime($cart_checkout_dates[$index]);
                ...
                if (($t1 >= $existing_checkin && $t1 < $existing_checkout) || 
                    ...
                    return redirect()->back()->with('error', 'You have already booked this room for the selected dates.');
                }
            }
        }
        ...
        // Add room details to session if no duplicate found
